...
const EditForm = ({ isOpen, onClose, onSuccess, booking, userId, currency = '$' }) => {
  ...
  const populateFormData = async () => {
    if (booking) {
      const roomId = booking.booked_rooms?.[0]?.room_id || '';
      const hallId = booking.booked_halls?.[0]?.hall_id || '';

      // Fetch booking guests
      try {
        const { data: bookingGuestsData, error } = await supabase
          .from('booking_guests')
          .select('guest_id, is_head_guest')
          .eq('booking_id', booking.id);

        if (error) throw error;

        const headGuestId = bookingGuestsData?.find(bg => bg.is_head_guest)?.guest_id || booking.guest_id || '';
        const additionalGuestIds = bookingGuestsData
          ?.filter(bg => !bg.is_head_guest)
          .map(bg => bg.guest_id) || [];

        setFormData({
          guest_id: headGuestId,
          additional_guests: additionalGuestIds,
          booking_type: booking.booking_type || 'room',
          adults: booking.adults || 1,
          kids: booking.kids || 0,
          check_in: booking.check_in ? booking.check_in.slice(0, 16) : '',
          check_out: booking.check_out ? booking.check_out.slice(0, 16) : '',
          room_type_id: booking.booked_rooms?.[0]?.rooms?.room_type_id || '',
          hall_type_id: booking.booked_halls?.[0]?.halls?.hall_type_id || '',
          room_id: roomId,
          hall_id: hallId,
          selected_services: [],
          coupon_code: booking.coupons?.coupon_code || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update booking guests - delete existing and insert new
      ...
      if (formData.booking_type === 'room' && formData.room_id) {
        const existingBookedRoom = booking.booked_rooms?.[0];
        ...
        const existingBookedHall = booking.booked_halls?.[0];
        if (existingBookedHall) {
          const { error: updateError } = await supabase
            .from('booked_halls')
            .update({
      ...
      onSuccess(bookingData[0]);
      onClose();
    } catch (error) {
      console.error('Error updating booking:', error);
      showToast('Failed to update booking: ' + error.message);
    } finally {
      setSaving(false);
    }
  };
          ...
          >
            ...
            <div className="bg-[#472F97] px-4 sm:px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-semibold text-white">Edit Booking #{booking.id}</h2>
                  <p className="text-[10px] sm:text-xs text-white/70">Update reservation details</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            >
              ...
              <div className="sticky bottom-0 bg-white border-t border-neutral-200 px-4 sm:px-6 py-4 flex items-center gap-3">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{saving ? 'Updating...' : 'Update Booking'}</span>
