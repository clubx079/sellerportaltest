...
const EditForm = ({ isOpen, onClose, onSuccess, bookedRoom, userId, currency = '$' }) => {
  ...
  useEffect(() => {
    if (isOpen && bookedRoom && userId) {
      fetchBookings();
      fetchRooms();
      populateFormData();
    }
  }, [isOpen, bookedRoom, userId]);
  ...
  const populateFormData = () => {
    if (bookedRoom) {
      setFormData({
        ...
        check_in: bookedRoom.check_in ? bookedRoom.check_in.slice(0, 16) : '',
        check_out: bookedRoom.check_out ? bookedRoom.check_out.slice(0, 16) : '',
        status: bookedRoom.status || 'booked'
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error updating booked room:', error);
      alert('Failed to update booked room: ' + error.message);
    } finally {
      setSaving(false);
    }
  };
          ...
          >
            ...
            <div className="bg-[#111111] px-4 sm:px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-semibold text-white">Edit Booked Room #{bookedRoom.id}</h2>
                  <p className="text-[10px] sm:text-xs text-white/70">Update room booking details</p>
                </div>
              </div>
              <button
                onClick={onClose}
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5">
                ...
                <div className="bg-neutral-50 rounded-xl p-4 border border-neutral-200">
                  <h3 className="text-sm font-semibold text-neutral-900 mb-2 flex items-center gap-2">
                    ...
                    Select Room <span className="text-neutral-500">*</span>
                  </h3>
                  <SearchableSelect
                    options={roomOptions}
