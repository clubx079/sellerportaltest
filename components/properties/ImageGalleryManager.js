...
// Last resort so a readable JPEG/PNG that merely carries a .heic name still uploads,
// while genuinely-HEIC bytes are rejected instead of stored as a broken image.
    ...
    try {
      ...
      if (typeIsHeic || (nameIsHeic && !typeIsReadable)) {
        ...
        try {
          ...
          // (heic-convert), which handles variants heic2any chokes on. NEVER fall back to
          // uploading raw HEIC bytes — that produced "completed" but unrenderable tiles.
          console.warn('heic2any failed, trying server convert:', err?.message || err)
          try {
            uploadFile = await serverConvertHeic(image.file)
          } catch (err2) {
            // Neither converter could read it. Upload only if the bytes are actually a
            // browser-viewable image that merely carries a .heic name; otherwise error.
              ...
              {image.status === 'queued' || image.status === 'uploading' ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center bg-tint-2">
                  ...
                  <div className="text-center p-4 relative z-10">
                    ...
                    <p className="text-xs font-semibold text-ink mb-2 px-1">{image.error || 'Upload failed'}</p>
                    <button
