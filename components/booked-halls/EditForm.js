...
import { X, Save, Users, Calendar, Building2, Clock } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import SearchableSelect from '@/components/ui/SearchableSelect';

const EditForm = ({ isOpen, onClose, onSuccess, bookedHall, userId, currency = '$' }) => {
  ...
  useEffect(() => {
    if (isOpen && bookedHall && userId) {
      fetchBookings();
      fetchHalls();
      populateFormData();
    }
  }, [isOpen, bookedHall, userId]);
  ...
  const populateFormData = () => {
    if (bookedHall) {
      setFormData({
        ...
        hall_id: bookedHall.hall_id || '',
        booking_basis: bookedHall.booking_basis || 'hourly',
        check_in: bookedHall.check_in ? bookedHall.check_in.slice(0, 16) : '',
        check_out: bookedHall.check_out ? bookedHall.check_out.slice(0, 16) : '',
        status: bookedHall.status || 'booked'
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error updating booked hall:', error);
      alert('Failed to update booked hall: ' + error.message);
    } finally {
      setSaving(false);
    }
  };
          ...
          >
            ...
            <div className="bg-[#472F97] px-4 sm:px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-semibold text-white">Edit Booked Hall #{bookedHall.id}</h2>
                  <p className="text-[10px] sm:text-xs text-white/70">Update hall booking details</p>
                </div>
              </div>
              <button
                onClick={onClose}
