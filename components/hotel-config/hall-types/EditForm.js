...
const EditForm = ({ isOpen, onClose, onSuccess, hallType, amenities, userId }) => {
  ...
  useEffect(() => {
    if (hallType) {
      ...
      setImagePreview(hallType.image || null);
      setImageFile(null);
    }
  }, [hallType]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      let imageUrl = hallType.image;

      if (imageFile) {
        if (hallType.image && hallType.image.includes('hotel-images')) {
          const oldPath = hallType.image.split('/hotel-images/').pop();
          if (oldPath) {
          ...
          >
            ...
            <div className="bg-[#472F97] px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-white">Edit Hall Type</h2>
                  <p className="text-xs text-white/70">Update hall details</p>
                </div>
              </div>
              <button
                onClick={onClose}
