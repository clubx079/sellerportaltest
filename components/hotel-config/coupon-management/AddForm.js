...
import { X, Save, Tag, Calendar, Percent, DollarSign, BedDouble, Briefcase, Search, ChevronDown } from 'lucide-react';
...
const AddForm = ({ isOpen, onClose, onSuccess, roomTypes, paidServices, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.coupon_period_start && formData.coupon_period_end) {
        // Extract just the date part from datetime-local value (YYYY-MM-DD)
        const startDate = formData.coupon_period_start.split('T')[0];
        const endDate = formData.coupon_period_end.split('T')[0];
          ...
          >
            ...
            <div className="bg-[#472F97] px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-white">Add Coupon</h2>
                  <p className="text-xs text-neutral-400">Create new discount coupon</p>
                </div>
              </div>
              <button
                onClick={onClose}
