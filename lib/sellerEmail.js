...
// Onboarding verification code — sent during signup when the seller picks the email channel.
export function emailVerificationCodeOnboarding({ code }) {
  const subject = 'Your DeelMap verification code'
  const html = shell({
    preheader: 'Your DeelMap verification code',
    body: `
      <h1 style="margin:0 0 12px;font-size:22px;font-weight:700;color:#1A1816;letter-spacing:-0.4px;line-height:1.25">Verify your account</h1>
      <p style="margin:0 0 8px;font-size:14px;line-height:1.65;color:#737370">Enter this code to continue setting up your DeelMap seller account:</p>
      <p style="margin:24px 0;font-size:30px;font-weight:700;letter-spacing:6px;color:#D03839">${escape(code)}</p>
      <p style="margin:0;font-size:12px;color:#A8A8A4;line-height:1.6">This code expires in 10 minutes. If you didn&rsquo;t request it, you can safely ignore this email.</p>`,
  })
  return { subject, html }
}
...
export async function sendSellerEmail({ to, subject, html }) {
  ...
  if (!to) return false
  try {
    // The Resend SDK returns API errors as { data, error } (it does NOT throw),
    // so a rejected send must be detected here — otherwise callers see "success".
    const { data, error } = await resend.emails.send({ from: FROM, to, subject, html })
    if (error) {
      console.error('[sellerEmail] send rejected:', error?.message || JSON.stringify(error))
      return false
    }
    return true
  } catch (e) {
