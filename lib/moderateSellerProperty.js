...
async function checkImage(url) {
  if (!url) return { pass: true }
  try {
    // Fetch and resize before sending to Groq — raw uploads can be several MB
    // and Groq rejects payloads above ~4MB. 512px JPEG @80% stays well under 200KB.
    const imgRes = await fetch(url)
    if (!imgRes.ok) {
      console.error('[seller-moderation] Image fetch failed:', imgRes.status, url)
      return { pass: true } // fail-open on network issues
    }
