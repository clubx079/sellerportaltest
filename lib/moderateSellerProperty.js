...
const PROFANITY_LIST = [
  ...
  'faggot', 'retard', 'motherfucker', 'bullshit', 'crap', 'hell',
]

function staticProfanityCheck(text) {
  if (!text?.trim()) return { pass: true }
  const lower = text.toLowerCase()
  const found = PROFANITY_LIST.find(word => {
...
async function checkText(text, address) {
  ...
  try {
    ...
    const reply = await groqChat([{
      role: 'user',
      content: `You are a strict content moderator for a real estate marketplace. Does the following text contain ANY of these: profanity, hate speech, sexual content, violence, threats, scam language, spam, or content that is clearly not appropriate for a professional real estate listing?\n\nReply with exactly "PASS" if the text is clean and professional, or "FAIL: <brief reason>" if it violates any of the above. Be strict — when in doubt, reply FAIL.\n\n${addressContext}${text.slice(0, 1200)}`
    }], 'llama-3.3-70b-versatile')
    if (reply.toUpperCase().startsWith('PASS')) return { pass: true }
    return { pass: false, reason: reply.replace(/^FAIL:\s*/i, '').slice(0, 200) }
  } catch {
    // AI unavailable — static check already passed, allow through
    return { pass: true }
  }
}
...
// Generate a signed URL so Groq can access private Supabase storage
async function getAccessibleImageUrl(originalUrl) {
  try {
    ...
    if (!match) return originalUrl // Not a Supabase storage URL, use as-is

    const bucket = match[1]
    const path = match[2].split('?')[0] // strip query params

    const { data, error } = await supabase.storage
      .from(bucket)
      .createSignedUrl(path, 60) // 60 second expiry — enough for Groq
...
async function checkImage(url) {
  if (!url) return { pass: true }
  try {
    const accessibleUrl = await getAccessibleImageUrl(url)
    const reply = await groqChat([{
      role: 'user',
      content: [
        { type: 'image_url', image_url: { url: accessibleUrl } },
        { type: 'text', text: 'You are a strict content moderator for a real estate marketplace. Is this image appropriate for a professional property listing? It must show property-related content (interior rooms, exterior, yard, street view, etc.).\n\nReply with exactly "PASS" if the image is appropriate, or "FAIL: <brief reason>" if it contains nudity, violence, graphic content, memes, screenshots, non-property photos, or anything clearly not related to real estate. Be strict — when in doubt, reply FAIL.' }
      ]
    }], 'llama-3.2-11b-vision-preview')
    if (reply.toUpperCase().startsWith('PASS')) return { pass: true }
    return { pass: false, reason: reply.replace(/^FAIL:\s*/i, '').slice(0, 200) }
  } catch {
    // If the image cannot be verified, reject it rather than pass
    return { pass: false, reason: 'Photo could not be verified — please re-upload the image' }
  }
}
