...
async function checkDocument(url, type = 'document') {
  ...
  try {
    // Tolerate relative /api/img URLs (same-origin uploads).
    const parsed = new URL(url, 'https://sell.deelmap.com')
    // DeelMap stores documents in Backblaze B2, served through the Cloudflare
    // signer at IMG_PUBLIC_BASE (e.g. cloudfare.apps.airosofts.com). Trust that
    // host (derived from env, so it survives a domain change) plus DeelMap's own
    // hosts and legacy Supabase storage.
    let imgHost = ''
    try { imgHost = new URL(process.env.IMG_PUBLIC_BASE || '').hostname } catch {}
    const trustedDomains = ['supabase.co', 'supabase.com', 'apps.airosofts.com', 'deelmap.com', 'backblazeb2.com']
    if (imgHost && !trustedDomains.includes(imgHost)) trustedDomains.push(imgHost)
