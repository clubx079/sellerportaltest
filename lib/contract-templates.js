 ...
 *
 *   Purchase Contract specific (wholesaling: OUR user is the Buyer, the
 *   counterparty is the outside property owner = Seller):
 *     - financing_type        'cash' | 'loan'  (the Buyer's funds)
 *     - seller_address        OUR user's (the Buyer's) address (profile default)
 *     - buyer_address         the owner's (Seller's) address
 ...
 *     - acceptance_deadline   ISO date the seller must accept by
...
export const TEMPLATE_CONFIG = {
  ...
  //   Wholesaling model: OUR user is the WHOLESALER locking up a deal, so they
  //   are the BUYER (First Party, signs first inline). The counterparty is the
  //   outside property OWNER = SELLER (Second Party, emailed to sign).
  //   The DocuSeal template's signing block was flipped to match via
  //   scripts/flip-purchase-roles.mjs (buyer_signature -> First Party,
  //   seller*_signature -> Second Party).
  //
  ...
  '3801788': /* local DocuSeal */ {
    ...
    description: "Use this to lock up a property from its owner. You sign as the Buyer; the owner signs as the Seller.",
    sortOrder: 1,
    fieldMap: {
      property_address:    'property_address',
      buyer_name:          'seller_name',         // wizard's counterparty = the OWNER = Seller on the contract
      purchase_price:      'sale_price',
      emd:                 'emd_amount',
      closing_date:        'closing_date',
      financing_type:      'source_of_funds',     // OUR user (the Buyer) brings the funds
      seller_address:      'buyer_address',        // wizard "your address" = OUR user (the Buyer)
      buyer_address:       'seller_address',       // wizard "owner's address" = the Seller
      ...
      co_seller_name:      'seller2_print_name', // optional co-owner on the seller print/sign line
    },
    autoFields: (ctx) => {
      const out = {
        contract_date: ctx?.todayISO || new Date().toISOString().slice(0, 10),
      }
      // OUR user is the Buyer — auto-fill their name in the top-of-doc buyer_name
      // and the buyer_print_name signing field (the field they'll sign next to).
      if (ctx?.sellerName) {
        out.buyer_name        = ctx.sellerName
        out.buyer_print_name  = ctx.sellerName
      }
      // Counterparty is the Seller (the owner) — pre-fill their print-name on the
      // seller signing line so they don't retype what we collected in Step 3.
      if (ctx?.buyerName) {
        out.seller1_print_name = ctx.buyerName
      }
      return out
    },
  },
