 ...
 *     - buyer_name            counterparty name (Seller on Purchase, Assignee on Assignment)
 ...
 *     - buyer_address         OUR user's address (profile default)
 *     - seller_address        property owner's address
 ...
 *     - acceptance_deadline   ISO date the seller must respond by
...
export const TEMPLATE_CONFIG = {
  // ─────────────────────────────────────────────────────────────────────────
  // 3718826 · (A to B) Deelmap Purchase Contract - Assignable
  //   User = BUYER. Counterparty = SELLER (property owner).
  //   21 named DocuSeal fields. Wizard pre-fills 14 of them; the remaining 7
  //   are signatures (handled by the DocuSeal signing flow itself).
  // ─────────────────────────────────────────────────────────────────────────
  '3718826': {
    slug: 'purchase',
    ...
    fieldMap: {
      ...
      buyer_name:          'seller_name',         // wizard's counterparty = Seller on this contract
      purchase_price:      'sale_price',
      emd:                 'emd_amount',
      closing_date:        'closing_date',
      financing_type:      'source_of_funds',
      // Newly collected:
      buyer_address:       'buyer_address',       // OUR user's address (profile default)
      seller_address:      'seller_address',      // property owner's address
