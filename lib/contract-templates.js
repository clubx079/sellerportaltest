 ...
 *
 *   Purchase Contract specific (OUR user is the Seller, the
 *   counterparty is the Buyer):
 *     - financing_type        'cash' | 'loan'  (the Buyer's funds)
 *     - seller_address        OUR user's (the Seller's) address (profile default)
...
export const TEMPLATE_CONFIG = {
  // ─────────────────────────────────────────────────────────────────────────
  // 3801788 · Deelmap Purchase Contract
  //   OUR user is the SELLER (First Party, signs first inline) — an actual
  //   property owner selling directly, or a wholesaler who double-closes.
  //   The counterparty is the BUYER (Second Party, emailed to sign).
  //   Signing block: seller*_signature -> First Party, buyer_signature ->
  //   Second Party (see scripts/flip-purchase-roles.mjs --to seller).
  //
  ...
  '3801788': /* local DocuSeal */ {
    ...
    fieldMap: {
      ...
      buyer_name:          'buyer_name',          // wizard's counterparty = the Buyer on the contract
      purchase_price:      'sale_price',
      emd:                 'emd_amount',
      closing_date:        'closing_date',
      financing_type:      'source_of_funds',     // the Buyer's funds
      seller_address:      'seller_address',      // OUR user's (the Seller's) address (profile default)
      buyer_address:       'buyer_address',       // the Buyer's address
      ...
      co_seller_name:      'seller2_print_name', // optional co-seller on the print/sign line
    },
    autoFields: (ctx) => {
      const out = {
        contract_date: ctx?.todayISO || new Date().toISOString().slice(0, 10),
      }
      // OUR user is the Seller — auto-fill their name in the top-of-doc seller_name
      // and the seller1_print_name signing field.
      if (ctx?.sellerName) {
        out.seller_name        = ctx.sellerName
        out.seller1_print_name = ctx.sellerName
      }
      // Counterparty is the Buyer — pre-fill their print-name on the buyer signing
      // line so they don't retype what we collected in Step 3.
      if (ctx?.buyerName) {
...
// a real, conditional signer (seller2 fields live on that role).
