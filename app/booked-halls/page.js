...
import { Plus, Edit2, Trash2, Eye, Search, X, Building2, Filter, ChevronLeft, ChevronRight, Clock, LogOut, Users } from 'lucide-react';
import AddForm from '@/components/booked-halls/AddForm';
import EditForm from '@/components/booked-halls/EditForm';
import ViewForm from '@/components/booked-halls/ViewForm';
import CheckoutModal from '@/components/booked-halls/CheckoutModal';
import DeleteConfirmModal from '@/components/booked-halls/DeleteConfirmModal';
import { getCurrentCurrencySymbol } from '@/lib/currency';

const BookedHallsManagement = () => {
  ...
  const [selectedBookedHall, setSelectedBookedHall] = useState(null);
  ...
  // Filter booked halls based on search term and filters
  const filteredBookedHalls = bookedHalls.filter(bookedHall => {
    const matchesSearch =
      bookedHall.id?.toString().includes(searchTerm) ||
      bookedHall.booking_id?.toString().includes(searchTerm) ||
      bookedHall.halls?.hall_number?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bookedHall.bookings?.guests?.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bookedHall.bookings?.guests?.phone?.includes(searchTerm);

    const matchesStatus = !filterStatus || bookedHall.status === filterStatus;
    const matchesCheckIn = !checkInDate || new Date(bookedHall.check_in).toISOString().split('T')[0] === checkInDate;
    const matchesCheckOut = !checkOutDate || new Date(bookedHall.check_out).toISOString().split('T')[0] === checkOutDate;
  ...
  const currentEntries = filteredBookedHalls.slice(indexOfFirstEntry, indexOfLastEntry);
  const totalPages = Math.ceil(filteredBookedHalls.length / entriesPerPage);

  // Handle delete
  const handleDeleteClick = (bookedHall) => {
    setSelectedBookedHall(bookedHall);
  ...
  const handleDeleteConfirm = async () => {
    try {
      const { error } = await supabase
        .from('booked_halls')
        .delete()
        .eq('id', selectedBookedHall.id)
        .eq('user_id', userId);

      if (error) throw error;

      setBookedHalls(prev => prev.filter(b => b.id !== selectedBookedHall.id));
      setShowDeleteModal(false);
      setSelectedBookedHall(null);
    } catch (error) {
      console.error('Error deleting booked hall:', error);
      alert('Failed to delete booked hall: ' + error.message);
    }
  };

  const handleCheckoutClick = (bookedHall) => {
    setSelectedBookedHall(bookedHall);
  ...
  // Calculate stats
  const totalBookedHalls = bookedHalls.length;
  const checkedInHalls = bookedHalls.filter(b => b.status === 'checked_in' || b.status === 'checkedin').length;
  const bookedHallsCount = bookedHalls.filter(b => b.status === 'booked').length;
  const checkedOutHalls = bookedHalls.filter(b => b.status === 'checked_out' || b.status === 'checkedout').length;

  // Get latest check-in and check-out times
  const getLatestCheckIn = () => {
    const checkedInBookings = bookedHalls.filter(b => b.status === 'checked_in' || b.status === 'checkedin');
  ...
  const getLatestCheckOut = () => {
    const checkedOutBookings = bookedHalls.filter(b => b.status === 'checked_out' || b.status === 'checkedout');
  ...
  return (
    <div className="space-y-3 md:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-lg md:text-xl font-semibold tracking-tight text-neutral-900">Booked Halls</h1>
            <p className="text-xs text-neutral-500 mt-0.5">Manage hall bookings & check-ins</p>
          </div>
          <button
          ...
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-base md:text-lg font-semibold text-neutral-900 mt-0.5">{totalBookedHalls}</p>
              </div>
          ...
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-base md:text-lg font-semibold text-neutral-900 mt-0.5">{checkedInHalls}</p>
                {latestCheckIn && (
          ...
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-base md:text-lg font-semibold text-neutral-900 mt-0.5">{bookedHallsCount}</p>
              </div>
              <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-[#472F97] flex items-center justify-center">
          ...
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-base md:text-lg font-semibold text-neutral-900 mt-0.5">{checkedOutHalls}</p>
                {latestCheckOut && (
                  <p className="text-[9px] md:text-[10px] text-neutral-400 mt-0.5 truncate">
        ...
        <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
              ...
              <div className="relative flex-1 sm:max-w-sm">
                ...
                <input
                  ...
                  placeholder="Search by booking ID, hall or guest..."
                />
                {searchTerm && (
                  <button
            ...
            <table className="w-full min-w-[900px]">
              <thead className="bg-neutral-50 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-left text-[10px] md:text-xs font-semibold text-neutral-500 uppercase tracking-wider whitespace-nowrap">Hall</th>
                  ...
                  <th className="px-3 py-2.5 text-left text-[10px] md:text-xs font-semibold text-neutral-500 uppercase tracking-wider whitespace-nowrap">Booking Basis</th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="9" className="px-3 md:px-5 py-8 md:py-10 text-center">
                      ...
                      <p className="text-neutral-500 text-xs md:text-sm font-medium">No booked halls found</p>
                      <p className="text-[10px] md:text-xs text-neutral-400 mt-1">Try adjusting your search or add a new booking</p>
                    </td>
                  </tr>
                ) : (
                  currentEntries.map((bookedHall, index) => (
                    <tr key={bookedHall.id} className="hover:bg-neutral-50 transition-colors">
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <span className="text-xs font-medium text-neutral-400">{indexOfFirstEntry + index + 1}</span>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <span className="text-xs font-semibold text-neutral-900">#{bookedHall.booking_id}</span>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          ...
                          <div>
                            <p className="text-xs font-medium text-neutral-900">
                              {bookedHall.halls?.hall_number || 'N/A'}
                            </p>
                            {bookedHall.halls?.hall_types?.title && (
                              <p className="text-[10px] text-neutral-400">{bookedHall.halls.hall_types.title}</p>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <div>
                          <p className="text-xs font-medium text-neutral-900">{bookedHall.bookings?.guests?.full_name || 'Walk-in Guest'}</p>
                          {bookedHall.bookings?.guests?.phone && (
                            <p className="text-[10px] text-neutral-400">{bookedHall.bookings.guests.phone}</p>
                          )}
                        </div>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <span className="text-xs text-neutral-600 capitalize">{bookedHall.booking_basis || 'N/A'}</span>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <span className="text-xs text-neutral-600">{formatDateTime(bookedHall.check_in)}</span>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <span className="text-xs text-neutral-600">{formatDateTime(bookedHall.check_out)}</span>
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium border ${getStatusColor(bookedHall.status)}`}>
                      ...
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        <div className="flex items-center justify-end gap-0.5">
                          ...
                          {(bookedHall.status === 'checked_in' || bookedHall.status === 'checkedin') && (
                            <button
                              onClick={() => handleCheckoutClick(bookedHall)}
                              className="p-1.5 rounded-lg hover:bg-neutral-100 text-neutral-500 transition-colors"
                              title="Checkout"
                            >
                          ...
                          <button
                            onClick={() => handleDeleteClick(bookedHall)}
                            className="p-1.5 rounded-lg hover:bg-neutral-100 text-neutral-500 transition-colors"
                            title="Delete"
                          >
          ...
          <div className="px-3 md:px-4 py-2 bg-neutral-50 border-t border-neutral-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="text-xs text-neutral-500">
              Showing <span className="font-medium text-neutral-700">{filteredBookedHalls.length > 0 ? indexOfFirstEntry + 1 : 0}</span> to{' '}
              <span className="font-medium text-neutral-700">{Math.min(indexOfLastEntry, filteredBookedHalls.length)}</span> of{' '}
              <span className="font-medium text-neutral-700">{filteredBookedHalls.length}</span> entries
            </div>
        ...
        <AddForm
          ...
          onSuccess={(newBookedHall) => {
            setBookedHalls(prev => [newBookedHall, ...prev]);
          }}
          userId={userId}
          currency={currency}
        />
        ...
        <DeleteConfirmModal
          ...
          itemName={`Booked Hall #${selectedBookedHall?.id}` || 'this booked hall'}
        />
    </div>
  );
};

export default BookedHallsManagement;
