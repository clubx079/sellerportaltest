...
import { Plus, Edit2, Trash2, Eye, Search, X, Building2, Layers, Home, Calendar, ShieldCheck, Filter } from 'lucide-react';
import DashboardLayout from '@/components/layout/DashboardLayout';
import AddForm from '@/components/hotel-config/halls/AddForm';
import EditForm from '@/components/hotel-config/halls/EditForm';
import ViewForm from '@/components/hotel-config/halls/ViewForm';
import DeleteConfirmModal from '@/components/hotel-config/halls/DeleteConfirmModal';
import HousekeepingModal from '@/components/hotel-config/halls/HousekeepingModal';
...
const HallsManagement = () => {
  ...
  const [bookedHallsToday, setBookedHallsToday] = useState(0);
  ...
  const fetchHalls = async () => {
    try {
      ...
      // Fetch employees separately and map them to halls
      ...
      const hallsWithEmployees = (hallsData || []).map(hall => ({
        ...hall,
        employees: hall.assigned_to ? employeesMap[hall.assigned_to] : null
      }));
  ...
  const fetchBookedHallsToday = async () => {
    try {
      ...
      if (error) throw error;
      setBookedHallsToday(data?.length || 0);
    } catch (error) {
      console.error('Error fetching booked halls today:', error);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      // Remove the deleted hall from state instead of refetching
  ...
  const getHousekeepingStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case 'inspected':
        return 'bg-blue-100 text-white border-blue-200';
      case 'out of service':
  ...
  return (
    <div className="space-y-3 md:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-lg md:text-xl font-semibold tracking-tight text-neutral-900">Halls</h1>
            <p className="text-xs text-neutral-500 mt-0.5">Manage hotel halls</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] md:text-xs text-neutral-500">Total Halls</p>
                <p className="text-base md:text-lg font-semibold text-neutral-900 mt-0.5">{halls.length}</p>
              </div>
          ...
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] md:text-xs text-neutral-500">Hall Types</p>
          ...
          <div className="bg-[#F5F3FF] rounded-xl border border-neutral-200 p-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] md:text-xs text-neutral-500">Booked Today</p>
                <p className="text-base md:text-lg font-semibold text-neutral-900 mt-0.5">{bookedHallsToday}</p>
              </div>
              <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-[#472F97] flex items-center justify-center">
        ...
        <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
          ...
          <div className="px-4 md:px-6 py-3 md:py-4 border-b border-neutral-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="relative flex-1 sm:max-w-sm">
              ...
              <input
                ...
                placeholder="Search by hall number, type, or floor..."
              />
              {searchTerm && (
                <button
            ...
            <table className="w-full min-w-[800px]">
              <thead className="bg-neutral-50 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-3 md:px-4 py-2.5 text-left text-[10px] md:text-xs font-semibold text-neutral-500 uppercase tracking-wider whitespace-nowrap">Hall Number</th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="7" className="px-3 md:px-5 py-6 md:py-10 text-center">
                      ...
                      <p className="text-neutral-500 text-base font-medium">No halls found</p>
                      <p className="text-xs text-neutral-500 mt-0.5">Try adjusting your search or add a new hall</p>
                    </td>
                  </tr>
                ) : (
                  filteredHalls.slice(0, entriesPerPage).map((hall, index) => (
                    <tr key={hall.id} className="hover:bg-neutral-50 transition-colors">
                      ...
                      <td className="px-3 md:px-4 py-2.5 whitespace-nowrap">
                        <div>
                          <p className="text-xs md:text-sm font-medium text-neutral-900">{hall.hall_types?.title || 'N/A'}</p>
                          {hall.hall_types?.short_code && (
                      ...
                      <td className="px-3 md:px-4 py-2.5 whitespace-nowrap">
                        <span className={`inline-flex items-center px-1.5 md:px-2 py-0.5 rounded-full text-[10px] md:text-xs font-medium border ${getHousekeepingStatusColor(hall.housekeeping_status)}`}>
                          {hall.housekeeping_status?.charAt(0).toUpperCase() + hall.housekeeping_status?.slice(1) || 'Clean'}
                        </span>
                      </td>
                      <td className="px-3 md:px-4 py-2.5 whitespace-nowrap">
                        <span className="text-xs md:text-sm text-neutral-700">
                          {hall.employees ? hall.employees.full_name : 'Unassigned'}
                        </span>
                      </td>
                      <td className="px-3 md:px-4 py-2.5 whitespace-nowrap">
          ...
          <div className="px-3 md:px-4 py-2 bg-neutral-50 border-t border-neutral-200">
            <div className="text-xs md:text-sm text-neutral-600">
              Showing <span className="font-medium text-neutral-700">{Math.min(filteredHalls.length, entriesPerPage)}</span> of{' '}
              <span className="font-medium text-neutral-700">{filteredHalls.length}</span> entries
            </div>
          </div>
        </div>
        ...
        <AddForm
          ...
          onSuccess={(newHalls) => {
            // Add the new halls to the state instead of refetching
            setHalls(prev => [...prev, ...newHalls]);
          }}
          floors={floors}
          hallTypes={hallTypes}
          userId={userId}
        ...
        <EditForm
          ...
          onSuccess={(updatedHall) => {
            // Update the hall in state instead of refetching
            setHalls(prev => prev.map(h =>
              h.id === updatedHall.id ? { ...h, ...updatedHall } : h
            ));
          }}
          hall={selectedHall}
          floors={floors}
        ...
        <DeleteConfirmModal
          ...
          itemName={selectedHall?.hall_number || 'this hall'}
        />
      </div>
  );
};
