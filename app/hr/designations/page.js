...
import { Plus, Edit2, Trash2, Eye, Search, X, Award, Star, TrendingUp, Users, ChevronDown } from 'lucide-react';
import AddForm from '@/components/hr/designations/AddForm';
import EditForm from '@/components/hr/designations/EditForm';
import ViewForm from '@/components/hr/designations/ViewForm';
import DeleteConfirmModal from '@/components/hr/designations/DeleteConfirmModal';
...
// Main Designation Management Page Component
const DesignationManagement = () => {
  ...
  // Fetch designations when userId is available
  useEffect(() => {
    if (userId) {
      fetchDesignations();
    }
  ...
  return (
    <div className="space-y-3 md:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-lg md:text-xl font-semibold tracking-tight text-neutral-900">Designation Management</h1>
            <p className="text-xs text-neutral-500">Manage employee designations</p>
          </div>
          <button
        ...
        <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-neutral-50 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-neutral-500 uppercase tracking-wider">Designation Name</th>
              ...
              <tbody className="divide-y divide-neutral-100">
                {loading ? (
                  <tr>
                    <td colSpan="3" className="px-4 py-8 text-center">
                      <div className="flex items-center justify-center gap-2 text-neutral-500">
                        ...
                        <span className="text-xs sm:text-sm font-medium">Loading designations...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan="3" className="px-4 py-8 text-center">
                      <Award className="w-7 h-7 md:w-8 md:h-8 text-neutral-300 mx-auto mb-2" />
                      <p className="text-neutral-500 font-medium text-xs sm:text-sm">No designations found</p>
                      <p className="text-[10px] sm:text-xs text-neutral-400 mt-1">Try adjusting your search or add a new designation</p>
                    </td>
                  </tr>
                ) : (
                      ...
                      <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3">
                        <div className="flex items-center gap-2 sm:gap-3">
                          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-neutral-100 flex items-center justify-center">
                            <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-neutral-600" />
                          </div>
          ...
          <div className="px-3 sm:px-4 py-2 sm:py-2.5 bg-neutral-50 border-t border-neutral-200">
            <div className="text-xs text-neutral-500">
              Showing {filteredDesignations.length} of {designations.length} entries
            </div>
          </div>
        </div>
        ...
        <DeleteConfirmModal
          ...
          itemName={selectedDesignation?.name || 'this designation'}
        />
      </div>
  );
};
